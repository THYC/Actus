...
public class CommandSignCmd implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext ctx) {
        ...
        if(src instanceof Player && src.hasPermission("actus.admin.sign.cmd")) { 
            Player player = (Player) src;
        
            if(!ctx.getOne("cmd").isPresent()){
            ...
            List<Text> cmd = new ArrayList<>();
            cmd.add(MESSAGE("&l&1[CMD]"));
            cmd.add(MESSAGE("&o&1--------"));
            cmd.add(MESSAGE("&l&4" + name));
            cmd.add(MESSAGE("&1Clique droit ici"));
            signData.set(Keys.SIGN_LINES,cmd );
            sign.offer(signData);
